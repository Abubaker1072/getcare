...
@section('content')
    <div class="flex flex-col md:flex-row md:items-end justify-between mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Hot Deal Management</h1>
            <p class="text-sm text-slate-500 mt-1 font-medium">Manage Hot Deal Products for Home Page & Hot Deals page (Max 8).</p>
        </div>
        <div class="mt-4 md:mt-0 flex space-x-3">
            <a href="{{ route('admin.hot-deals') }}" class="flex items-center px-5 py-2 bg-white text-slate-700 border border-slate-200 rounded-xl text-sm font-semibold hover:bg-slate-50 transition-all duration-200">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-white border-b border-slate-100 text-xs uppercase tracking-widest text-slate-400">
                        ...
                        <th class="p-5 font-bold text-center">Hot Deal</th>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td class="p-5 text-center">
                            @if($product->homepageHotDeal)
                                <span class="bg-rose-50 text-rose-600 border border-rose-100 px-2.5 py-1 rounded-lg text-xs font-bold tracking-wide">Yes</span>
                            @else
                        ...
                        <td class="p-5 text-right">
                            <form action="{{ route('admin.hot-deal-management.toggle', $product->id) }}" method="POST" class="inline-block">
                                @csrf
                                <button type="submit" class="px-4 py-2 text-xs font-semibold rounded-lg transition-colors
                                    {{ $product->homepageHotDeal ? 'bg-rose-50 text-rose-600 hover:bg-rose-100' : 'bg-orange-50 text-orange-600 hover:bg-orange-100' }}">
                                    {{ $product->homepageHotDeal ? 'Remove Hot Deal' : 'Add Hot Deal' }}
                                </button>
                            </form>
                        </td>
                    </tr>
