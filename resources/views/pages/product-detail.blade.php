...
<div class="w-full pb-24 font-sans text-gray-800">
    ...
    <section class="py-12 bg-white overflow-hidden border-t border-b border-gray-100" id="reviews">
        <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row items-center justify-between gap-4 mb-12">
                <div class="text-center md:text-left">
                    <h2 class="text-3xl sm:text-4xl font-serif text-gray-900 mb-2">Customer Reviews</h2>
                    <p class="text-xs text-gray-500 font-medium">Hear from our clients about their skincare routine transformations.</p>
                </div>
                
                {{-- Write Review Button --}}
                <button type="button" onclick="window.openReviewModal({{ $product->id }}, '{{ addslashes($product->name) }}', '{{ asset('storage/' . ($product->cover_image ?? $product->image)) }}')" 
                        class="bg-[#1a1a1a] hover:bg-black text-white px-6 py-3 rounded-full font-bold text-xs tracking-wider transition-colors shadow-sm whitespace-nowrap">
                    WRITE A REVIEW
                </button>
            </div>
            ...
            @if($product->reviews && $product->reviews->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-gray-50 p-6 rounded-2xl border border-gray-100 flex flex-col hover:shadow-md transition-shadow">
                        ...
                        <div class="flex items-center gap-3 mt-auto pt-4 border-t border-gray-200">
                            <div class="w-8 h-8 rounded-full bg-gray-250 flex items-center justify-center text-gray-600 font-bold text-xs">
            ...
            @else
                {{-- Empty State --}}
                <div class="text-center py-12 px-4 bg-gray-50 rounded-3xl border border-gray-100 max-w-lg mx-auto">
                    <span class="text-4xl">✨</span>
                    <h3 class="font-serif text-lg text-gray-900 mt-3 mb-1">No Reviews Yet</h3>
                    <p class="text-sm text-gray-500 mb-6">Be the first to share your experience with this premium product!</p>
                    <button type="button" onclick="window.openReviewModal({{ $product->id }}, '{{ addslashes($product->name) }}', '{{ asset('storage/' . ($product->cover_image ?? $product->image)) }}')" 
                            class="border border-[#1a1a1a] text-[#1a1a1a] hover:bg-gray-50 px-6 py-2.5 rounded-full font-bold text-xs tracking-wider transition-all">
