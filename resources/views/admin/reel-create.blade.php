...
<div class="max-w-3xl mx-auto">
    <div class="flex flex-col md:flex-row md:items-end justify-between mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Add New Reel</h1>
            <p class="text-sm text-slate-500 mt-1 font-medium">Create and publish a new video reel on the home page.</p>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] overflow-hidden p-8">
        <form action="{{ route('admin.reels.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Video File (.mp4, .mov, .webm) <span class="text-rose-500">*</span></label>
                    <input type="file" name="video" accept="video/*" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all" required>
                    <p class="text-xs text-slate-400 mt-1">Maximum file size: 20MB.</p>
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Custom Thumbnail / Poster (Optional)</label>
                    ...
                    <p class="text-xs text-slate-400 mt-1">Optional image displayed before the video plays.</p>
                </div>
            </div>

            <div class="mb-6">
                <label class="block text-sm font-bold text-slate-700 mb-2">Caption</label>
                <input type="text" name="caption" value="{{ old('caption') }}" placeholder="Used by the professionals..." class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all">
            </div>
