        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                @if($product->reference_image)
                                    <div class="mb-3">
                                        <img src="{{ asset('storage/' . $product->reference_image) }}" alt="Reference Image" class="w-full max-w-xs h-32 object-cover rounded-xl border border-slate-200">
                                    </div>
                                @endif
                            ...
                            <div class="flex flex-col gap-4">
                                <div>
                                    ...
                                    <input type="text" name="reference_title" value="{{ old('reference_title', $product->reference_title) }}" placeholder="e.g. Pure & Natural" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500/20 outline-none">
                                </div>
                                <div>
                                    <label class="block text-sm font-bold text-slate-700 mb-2">Reference Text</label>
                                    <textarea name="reference_text" rows="2" placeholder="e.g. Elevate your daily routine with our premium collection." class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500/20 outline-none">{{ old('reference_text', $product->reference_text) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="mb-8 flex flex-wrap gap-6">
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="rounded border-slate-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 w-5 h-5 transition duration-150" {{ $product->is_active ? 'checked' : '' }}>
                    <span class="ml-2.5 text-sm font-bold text-slate-700">Active (Visible to customers)</span>
                </label>
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox" name="is_on_sale" value="1" class="rounded border-slate-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 w-5 h-5 transition duration-150" {{ $product->is_on_sale ? 'checked' : '' }}>
